...
class TranscriptControllerIT extends FacadeIT {
    ...
    var studentUser =
        appUserRepository.save(
            JAppUser.builder()
                ...
                .lastName("Student")
                .email("transcript-" + UUID.randomUUID() + "@example.com")
                .password("hashed")
                .role(UserRole.STUDENT)
                .createdAt(Instant.now())
                .build());

    token = jwtService.generateToken(studentUser.getId(), studentUser.getRole());
    studentRepository.save(
        JStudent.builder().id(studentUser.getId()).std("STD24060").promotion(promotion).build());
    ...
    var teacher =
        appUserRepository.save(
            JAppUser.builder()
                .firstName("Teacher")
                .lastName("Transcript")
                .email("transcript-teacher-" + UUID.randomUUID() + "@example.com")
                .password("hashed")
  ...
  @Test
  void getTranscript_should_return_the_single_year_with_the_recorded_grade() {
    var response =
        restTemplate.exchange(
            "/students/" + studentId + "/transcript?academicYear=2024",
            HttpMethod.GET,
            new HttpEntity<>(authHeaders()),
            TranscriptResponse.class);

    assertEquals(200, response.getStatusCode().value());
    assertTrue(response.getBody().complete());
    assertEquals(16.00, response.getBody().generalAverage());
    assertEquals(6, response.getBody().totalCredits());
  }

  @Test
  void getFullTranscript_should_include_the_2024_year() {
    var response =
        restTemplate.exchange(
            "/students/" + studentId + "/transcript/full",
            HttpMethod.GET,
            new HttpEntity<>(authHeaders()),
            FullTranscriptResponse.class);

    assertEquals(200, response.getStatusCode().value());
    assertEquals(1, response.getBody().years().size());
    assertEquals(2024, response.getBody().years().get(0).academicYear());
    assertTrue(response.getBody().complete());
  }
}

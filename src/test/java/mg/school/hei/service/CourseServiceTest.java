...
class CourseServiceTest {
  ...
  void create_should_save_and_return_the_course() {
    when(courseRepository.save(any()))
        .thenAnswer(
            inv -> inv.getArgument(0, JCourse.class).toBuilder().id(UUID.randomUUID()).build());
  ...
  void delete_should_reject_when_course_has_assignments() {
    ...
    when(courseRepository.findById(id))
        .thenReturn(
            Optional.of(JCourse.builder().id(id).ref("PROG4").title("Q").credits(6).build()));
    var assignment = JCourseAssignment.builder().course(JCourse.builder().id(id).build()).build();
